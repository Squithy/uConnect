...
if [[ -e /fs/etfs/NO_GPS && $VARIANT_ECELL = "NO" ]]; then
   ...
   qwaitfor /dev/ser2
   # Start GPS driver
   nice -n -1 vdev-flexgps -P/dev/gpio/GPSReset -d/hbsystem/multicore/navi/g -b9600 -c/dev/ser2 -C115200 -g$NAV_DIR/g5.cfg -L20 -r1 -R  &
fi

# Start NDR
if [ ! -d /fs/etfs/usr/var/ndr ]; then
    mkdir -p /fs/etfs/usr/var/ndr
fi
