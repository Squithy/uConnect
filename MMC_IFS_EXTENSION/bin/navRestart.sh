...
if [[ -h $NAV_DIR/NNG/sys.txt ]]; then
  rm $NAV_DIR/NNG/sys.txt
  ln -sfP /fs/mmc0/app/share/nav/sys.txt $NAV_DIR/NNG/sys.txt
fi
...
cd $NAV_DIR/ON
if [ ! -e /dev/serv-mon/com.harman.service.Navigation ]; then
   echo Restarting FiatOpenNavController
   FiatOpenNavController --nobreak --disable-watchdog  --tp=/fs/mmc0/app/share/trace/nav.hbtc --bp=/HBpersistence & 
else
   echo FiatOpenNavController still alive
fi
