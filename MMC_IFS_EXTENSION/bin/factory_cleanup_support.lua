...
local can_utils = require "cmc.can_utils"
...
-- Elvis 1340830:  Write the default value of US_METRIC to the CAN bus.
-- Currently, the only way to get the default value is to reset the pps key; the next read returns the default.
assert(can_utils.open(can_utils.ctrlChan, "wc"))
local us_metric    = service.invoke("com.harman.service.PersistentKeyValue", 'resetkey', {["keys"] = {"US_METRIC"}})
if (us_metric ~= nil) and (us_metric.res == "Success") then
   us_metric = service.invoke("com.harman.service.PersistentKeyValue", 'read', {["key"] = "US_METRIC"})
   if us_metric ~= nil then
      print("factory_cleanup_support.lua: resetting us_metric to ", us_metric.res)
      can_utils.write({CFG_FEATURE=0x0B,CFG_STAT_RQ=0x03,CFG_SET=us_metric.res}) -- metric=1, US= 0
   else
      print("factory_cleanup_support.lua: reset failed us_metric to ", us_metric.res)
   end
end
can_utils.close()
